...
/*   Updated: 2023/11/02 15:48:34 by truello          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	print_format(t_format *format, va_list *args)
{
	if (format->specifier == 'c')
		...
		return (print_hexa(format, va_arg(*args, unsigned int), 0));
	else if (format->specifier == 'X')
		return (print_hexa(format, va_arg(*args, unsigned int), 1));
	return (0);
}
...
static int	parse_character(const char *s, t_format *format, va_list *args)
{
	...
	else if (is_width(*s))
	{
		...
		if (*s == '*')
			return (1);
		return (get_digit_len(s));
	}
	else if (is_precision(*s))
	{
		format->precision = parse_num(s + 1, args);
		if (*(s + 1) == '*' || !ft_is_digit(*(s + 1)))
			return (1);
		return (1 + get_digit_len(s + 1));
	}
	else
		return (0);
}
